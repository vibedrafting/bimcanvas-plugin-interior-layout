...
description: |
  ...
  设计知识本体（闭合预检/自改图边界/识图维度等）在 references，经 load-design-knowledge 注入。
...
1. **【必须】**通过 `Skill` 加载 `load-design-knowledge`（**`level: L2`**，`roomType` 按房间类型）—— 施工 + 自评必读：L1 注入 `空间设计.md` + `家具放置.md` + 房间策略 + `module_library.json`；L2 追加 `design_evaluation.md`（**识图自评的维度判据与各维「视觉检查」问法来源**）。（闭合施工预检 / 修正阶梯 / 自改图边界 = 施工方法，**方法本体已内联在本 skill 的 Step C/E**，不在 references。）
...
2. **残量处置**：对模块库 relation_rules 中标注"**布置后执行顶角规则检查**"的条目逐条执行其动作（平移柜体组贴角 / 扩宽消隙 / 把残量挪向门口·通道侧——动作判据见 `家具放置.md` 顶角闭合处置 + 房间策略）。处置后仍存在的残量，**必须登记进施工简报「保留空段与关键留白」节**（残量在哪侧、为何可接受）——未登记的残量 = 无意识窄缝，不许留。
...
> **可选家具收束纪律（R1.16/17/18）**：收束时不擅自切换到 `variantContext` 未指向的候选类型、不改主家具墙面关系（越界=语义级改图，停下上报）；**禁"居中布置"**——每件可选家具必写精确坐标与来源；睡眠组必写窗侧锚坐标 + 组装顺序（见 Step D2#3）。空间充裕时本可放的可选家具不无故省略（候选位清单见 `bedroom.md`）。
