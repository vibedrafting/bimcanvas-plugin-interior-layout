...
- **调用①设计品质（Layer 2）**：prompt 内**结构化列 5 个聚焦小问**——动线设计（含通行可达：实走核实、禁脑补绕行）/ 空间意图 / 功能叙事 / 空间节奏 / 家具最优布局各一问，问法取品质维度（load-design-knowledge 注入）各维「视觉检查」，**要求逐项分别作答**。**采光遮挡不入识图小问**——识图对采光易误报（把窗帘/矮件误判遮挡），走坐标判定（见 design_evaluation 采光维「识图盲区」）。**id→家具名 图例由 `canvas_vision`（截图+识图模式）自动注入——prompt 内不必手动附模块清单**；如需坐标背景（通道宽 / 墙长等坐标可判项），只附必要数值即可。
