...
2. **逐条跑本次加载到的坐标自查锚**：把 load-design-knowledge 本次注入的 references 里**所有 `【自查·坐标】` / `【自查·validate】` 锚，逐条按台账核验**（窗帘满墙 / 柜列填满 / 标准件优先 / 主家具贴墙 / 可选家具贴邻 gap=0 / 窗侧锚 gap=0含垂直床角部 / 顶角闭合残量 / 台盆贴角 / 坐便器居中 / 沙发-电视距… **以本次实际加载到的锚为准，不凭记忆罗列**）。**任一不过 = 自检不过、回 Step C/E 修正、不进 Step F。** 判据本体在 reference 自查锚，本步只执行核验、不另立判据。
...
> **可选家具收束纪律（R1.16/17/18）**：收束时不擅自切换到 `variantContext` 未指向的候选类型、不改主家具墙面关系（越界=语义级改图，停下上报）；**禁"居中布置"**——每件可选家具必写精确坐标与来源；睡眠组必写窗侧锚坐标 + 组装顺序（窗侧锚 gap=0 判据见 load-design-knowledge 注入的 bedroom 窗侧锚自查锚，Step D2 核验）。空间充裕时本可放的可选家具不无故省略（候选位清单见 load-design-knowledge 注入的房型范式）。
