...
description: |
  ...
  一个变体：register_variant（隐藏目录 _{slug}）+ (按需) zones.json + 施工简报（闭合预检/扣减账本/fallback）
  + 按图施工 + 落位自检 + validate + 2 次定点识图自评（设计品质5维/通用品质4项）+ 自优化（报警逐条
  处置 + 可选家具补全含联动置换）+ **成功(ok:true)才 reveal_variant 转正可见** + 返回结构化 factsheet。
  认输/失败留隐藏不进轮播(fail-safe)。供 placement 壳 agent 加载执行。
  设计知识本体（放置法则/识图维度等）由 load-design-knowledge 按阶段注入（不在本 skill）。
allowed-tools: Read, Write, Edit, Skill, mcp__interior-layout__register_variant, mcp__interior-layout__reveal_variant, mcp__interior-layout__get_zone_boundaries, mcp__canvas__validate_layout, mcp__canvas__canvas_vision
---
...
- 调 `mcp__interior-layout__register_variant({ designZoneId, slug, visible: false, leafCount, summary: variantDirection })` 建**隐藏**目录骨架(`_{slug}`)。返回的 `leafPaths` 是各叶子 modules.json 路径。
  - **WHY 隐藏注册(fail-safe)**:候选先隐藏,**只有 Step H 正常完成(`ok:true`)才揭示可见**(`reveal_variant`)；认输 / 崩溃 / 超时一律留隐藏、不进 Web 采纳轮播,杜绝"系统已判失败的破布局仍可被用户采纳"(M1 实测 north-sanctuary 认输后产物残留可见之坑)。
...
模块全部落位后、调 `validate_layout` 前（坐标级，不依赖识图）：

1. **建逐墙坐标台账**：每件贴墙家具，枚举沿墙两端邻接物——垂直墙（顶死？）/ 相邻家具（贴合？）/ 残量（宽度、在哪侧）。台账为下面逐条核验自查锚提供坐标事实。
2. **逐条跑本次注入的坐标自查锚**：把 **load-design-knowledge 本次注入的**所有 `【自查·坐标】` / `【自查·validate】` 锚，逐条按台账核验（通用类如 满墙 / 填满有效段 / 标准件优先 / 主家具贴墙 / 可选家具贴邻 gap=0 / 成套组锚点 gap=0 / 顶角闭合残量，加注入的**房型专项锚**；**以本次实际加载到的锚为准，不凭记忆罗列、不写参考文件名**）。**任一不过 = 自检不过、回 Step C/E 修正、不进 Step F。** 判据本体在 load-design-knowledge 注入的知识里，本步只执行核验、不另立判据。
3. **残量处置**：对模块库 relation_rules 标注"**布置后执行顶角规则检查**"的条目逐条执行其动作（平移贴角 / 扩宽消隙 / 残量挪门口·通道侧）；处置后仍存的残量**必须登记进施工简报「保留空段」节**——未登记的残量 = 无意识窄缝，不许留。

**【边界】**本步核验一切**坐标可判**项（贴墙 / 邻接 / 残量 + 加载到的所有坐标自查锚）——只有坐标判不了的（整体观感 / 渲染异常 / 隐私节奏好不好）才留给 Step F 识图；**坐标可判的缺陷不得下放识图**（视觉弱会假"全过"，Test#4/#5 窗帘没满墙、梳妆台死缝被识图放行即此）。

> WHY：判据**单一归宿在 reference 自查锚**，placement 是"跑加载到的锚"的执行器——知识层加一条坐标锚即自动纳入本步核验，**不必回头改本 skill**（知识层为护城河、机制为 commodity 的落地：加规则只动 reference）。
...
- **合同内 fallback（可自动执行）**：施工简报 `## 合同内 fallback` 已写、且当前失败原因与触发条件一致时，执行其中可自动的下一档方案。**【必须·降级前穷尽几何级】降级主家具尺寸等级（如主卧 1800→1500 床）前，必须先完成几何级修正（平移 / 重定位 / 重排成套家具组贴其锚点）并确认几何上确实放不下——禁止因一次未贴锚优化的初始落位报 validate 错就触发降级 fallback**。执行前确认仍满足全部边界。统一记 `[自动适配]`。
...
- **调用①设计品质（Layer 2）**：prompt = 下方 **v10 设计品质识图模板**，**逐字原样发送**——不得改写问法、增删条目、重排顺序、或自换框架。这是多轮对照真实识图模型打磨出的**稳定仪器**，临场发挥重拼会让分析质量回退（实测：开放/自拼问法漏判「不可达」、误报「封窗」；冻结模板才稳）。id→家具名图例由 `canvas_vision` 自动注入，模板内不含、也不必手动附；**采光不在模板内**（识图对采光易误报，走坐标判定，见 design_evaluation 采光维「识图盲区」）。

  ````text
  你是经验丰富的验房师，只凭图上看得到的证据下判断。看这张家具平面图（图例已给 id = 家具名，按名字认每件是什么），逐项找硬伤。

  规矩：每项先写"看到<证据>"再判定，一两句话、不铺陈；看不出就写"看不出"；只报硬伤、不夸优点。
  示例 —— `北墙: 看到最长一面实墙大半截裸露、不像通道 → 最优墙闲置`

  1.【可达 + 动线】每道门（尤其通往里间 / 凹区的门）：从房间空地一步步实走过去——沿途和门口紧贴的是什么？有没有被床 / 柜挡断、留不下一个人过的口子？只看门扇能不能开不算数，要看人走不走得到；走不到 = 那间作废，最严重。再看去床 / 衣柜等常用路径有没有被迫绕行折返。
  2.【家具最优墙】最长的几面实墙分别被什么占？最重要的家具（床）有没有落在它的最优墙，还是被挤到短墙、把最长的墙让给了次要家具（如衣柜）？
  3.【功能叙事】同一面墙 / 同一区域的家具是不是连贯一组？有没有被无关家具插断（如连续衣柜被梳妆台截断）？
  4.【空间意图】有没有说不出功能的剩余区域？每块空区像通道 / 缓冲，还是"家具放完剩下的"？
  5.【空间节奏】从门口往里看：私密的（床）在深处、过渡的（更衣 / 收纳）在入口侧吗？有没有私密功能怼在门口、一进门一览无余，或入口没有缓冲？
  6.【最严重】这套布局最致命的 1–2 个硬伤。

  精确尺寸、哪件该挪、"本可以更大"——交坐标核验，你看着不对标出来即可，不下结论。
  ````

  > **冻结纪律**：上方模板逐字发送。design_evaluation 各维判据 / WHY（load-design-knowledge 注入）供你**理解维度 + Step G 处置识图报警时坐标复算**，**不是现场拼 prompt 的素材**。模板需改（如补维、换措辞）须重新对照真实模型打磨成新版再冻结，不在执行期临场改。
- **调用②通用品质（Layer 1.5）**：prompt = 下方 **v4 通用品质识图模板**，**逐字原样发送**（冻结纪律同上：不改写问法、不增删条目、不重排）。同样多轮对标真实模型打磨：强制逐端枚举 + 深度视错觉防误报，8+3 跑零误报且稳定。id→家具名图例由 `canvas_vision` 自动注入，模板内不含、不必手动附。**几 cm 级精确贴合 / 小缝（如柜端差一两百 mm 没顶墙）识图看不见——归坐标台账，不在本模板强求**。

  ````text
  你是经验丰富的验房师，只挑"摆放规整 + 空间浪费"的毛病——不评设计好坏，只看放得齐不齐、贴没贴墙、有没有空着浪费。看这张家具平面图（图例已给 id = 家具名），逐项找硬伤。

  规矩：每项先写"看到<证据>"再判定，一两句话、不铺陈；看不出就写"看不出"；只报硬伤、不夸优点。
  示例 —— `北墙: 看到大半截墙裸着、不像过道 → 大段墙面浪费`

  1.【靠墙完整性】每件大家具背面都贴墙吗？有没有悬空离墙、或本该连成一排的柜子中间无故断开？
  2.【相邻空隙】相邻两件之间有没有"既不是通道、也塞不下东西"的无用窄缝？（深浅不同的家具贴同一面墙、或一件垂直抵着另一件端头，都不算缝——只报中间真空出一块地的缝）
  3.【沿墙逐端到头】把每排靠墙的家具 / 柜列逐排过一遍，每排一行、说清左右两端各抵着什么：顶到拐角的垂直墙、抵着另一件家具、还是差一截没到头露出空墙？（别拿"挨着旁边柜子"搪塞"到没到墙尽头"）哪端差一截没到头就点出来。
  4.【空间利用】只看本房间内部（别算门外 / 隔壁房间）：有没有整面长墙、或一大段墙裸着没家具、又不像通道 / 缓冲？在哪？

  几 cm 级的贴合 / 小缝看不准就标"拿不准"、交坐标核验；你只报明显看得出的不规整与浪费。
  ````

**纪律**：
- **【必须·禁大杂烩】**一次调用 = 一组结构化小问逐项作答，**不是**"几维有没有问题"一句话（实测：开放大问返回对错混杂不可用，定点小问全部有效）。
...
- **【必须·识图失效=视觉缺失，禁编造】**识图失效有两类：① `canvas_vision` 返回错误/连接失败/异常/空结果（工具层失败，本次实测 aoment 宕机即此类）② 看见空房/明显错乱（内容失败）。任一类即失效 → 换 viewport 或问法重试一次；**总预算 ≤4 次**（2 正式 + 2 重试），仍失效：**禁止凭坐标/简报编造任何维度判定、禁报"全过/零报警"，该轮所有维度一律标 `[视觉验证缺失]`**，坐标级检查（D2/validate）标准不降。
  > WHY：你无 vision，识图失败时若凭坐标写出"各维全过"，那是**凭空捏造的假信号**——比没有自检更危险，会让用户误以为方案经过视觉把关而误导终选（本次实测两方案在 aoment 宕机下都谎报"全过"）。
...
> **可选家具收束纪律（R1.16/17/18）**：收束时不擅自切换到 `variantContext` 未指向的候选类型、不改主家具墙面关系（越界=语义级改图，停下上报）；**禁"居中布置"**——每件可选家具必写精确坐标与来源；成套家具组必写其锚坐标 + 组装顺序（具体锚与 gap 判据见 load-design-knowledge 注入的房型自查锚、Step D2 核验）。空间充裕时本可放的可选家具不无故省略（候选位清单见 load-design-knowledge 注入的房型范式）。
...
- **【必须·成功才揭示可见】正常完成(`ok:true`)前**:调 `mcp__interior-layout__reveal_variant({ designZoneId, slug })` 把本候选从隐藏 `_{slug}` 转正为可见 `{slug}`,使其进入用户采纳轮播。**仅 `ok:true` 调此工具**;下面任一认输路径(`ok:false`)**绝不调用**——候选留隐藏 `_{slug}`、不进轮播(fail-safe)。`reveal_variant` 只转正可见、不翻 `adopted` 指针(终选仍归用户)。
