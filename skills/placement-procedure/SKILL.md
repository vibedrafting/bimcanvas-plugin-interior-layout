...
- **调用①设计品质（Layer 2）**：prompt = 下方 **v9 设计品质识图模板**，**逐字原样发送**——不得改写问法、增删条目、重排顺序、或自换框架。这是多轮对照真实识图模型打磨出的**稳定仪器**，临场发挥重拼会让分析质量回退（实测：开放/自拼问法漏判「不可达」、误报「封窗」；冻结模板才稳）。id→家具名图例由 `canvas_vision` 自动注入，模板内不含、也不必手动附；**采光不在模板内**（识图对采光易误报，走坐标判定，见 design_evaluation 采光维「识图盲区」）。
