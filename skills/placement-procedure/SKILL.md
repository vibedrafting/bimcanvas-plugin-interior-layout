...
description: |
  ...
  设计知识本体（放置法则/识图维度等）由落地阶段按需注入上下文（不在本 skill）。
...
2. **逐条跑本次已注入的坐标自查锚**：把**本次落地知识里已注入上下文的**所有 `【自查·坐标】` / `【自查·validate】` 锚，逐条按台账核验（通用类如 满墙 / 填满有效段 / 标准件优先 / 主家具贴墙 / 可选家具贴邻 gap=0 / 成套组锚点 gap=0 / 顶角闭合残量；**房型专项判据由已注入的房型知识自带**——**以本次实际加载到的锚为准，不凭记忆罗列、不指名具体房型**）。**任一不过 = 自检不过、回 Step C/E 修正、不进 Step F。** 判据本体在所注入的知识里，本步只执行核验、不另立判据。
...
- **调用①设计品质（Layer 2）**：prompt 内**结构化列 6 个聚焦小问**——动线设计 / 空间意图 / 功能叙事 / 空间节奏 / 采光通风 / 家具最优布局各一问，问法取**已注入的**品质维度各维「视觉检查」，附本方案关键坐标背景（主家具墙面/通道宽度等），**要求逐项分别作答**。
...
> **可选家具收束纪律（R1.16/17/18）**：收束时不擅自切换到 `variantContext` 未指向的候选类型、不改主家具墙面关系（越界=语义级改图，停下上报）；**禁"居中布置"**——每件可选家具必写精确坐标与来源；成套家具组必写其锚坐标 + 组装顺序（具体锚与 gap 判据由已注入的房型知识定义、Step D2 核验）。空间充裕时本可放的可选家具不无故省略（候选位清单由已注入的房型知识提供）。
