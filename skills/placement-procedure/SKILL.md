...
3. **窗侧锚坐标自检**：双床头柜分支（睡眠组贴窗侧锚）落位后，取窗帘占位结束线坐标与窗侧床头柜的贴窗边坐标，核 `gap == 0mm`（沿采光轴方向：南窗则比 `窗帘 Y_max` 与 `窗侧床头柜 Y_min`；东/西窗则比对应 X 坐标）。**gap > 0 即窗侧空段违规**，必须把睡眠组整体重排贴回窗侧锚（窗帘→窗侧柜 gap=0→床→使用侧柜），剩余墙段只允许留在使用侧。**禁止**保留该空段、更禁止在简报里用"窗前通行留白/窗前缓冲"之类措辞把它合理化——窗帘盒与床之间的空段是无功能空段，不是有意留白。

4. **坐标级硬断言（逐条核 bounds，任一不过 = 自检不过、回 Step C/E 修正、不进 Step F）**：① **窗帘满墙**——窗帘沿墙跨度 == 所靠墙有效宽（到两端墙），被角落家具逼成窗宽 = 不过；② **柜列填满**——柜列占用 == 有效段，剩余须有简报登记留白且尺寸量纲匹配功能（超量判欠填满）；③ **标准件优先**——等长于标准件的段必含该标准件、无多定制拼标准长；④ **主家具贴墙**——背边贴墙无悬空。判据本体见 load-design-knowledge 注入的放置法则 / 品质维度，本步只做坐标核验。
...
- **合同内 fallback（可自动执行）**：施工简报 `## 合同内 fallback` 已写、且当前失败原因与触发条件一致时，执行其中可自动的下一档方案；执行前确认仍满足全部边界。统一记 `[自动适配]`。
