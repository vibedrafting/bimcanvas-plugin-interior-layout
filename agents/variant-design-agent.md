...
2. `mcp__interior-layout__load_semantic_plan({zoneId: designZoneId, tag: "spatial-skeleton"})` —— **不传 variantId**
...
4. `mcp__interior-layout__get_zone_boundaries({zoneIds: [designZoneId]})` —— 取设计区与其叶子分区边界
...
- **【禁止】**调用 `mcp__interior-layout__load_reference_analysis` / `mcp__interior-layout__save_reference_analysis` / `mcp__canvas__analyze_image`（multi-plan 与 reference 互斥）
