...
tools: Read, Write, Edit, Glob, Grep, Skill, mcp__canvas__validate_layout, mcp__canvas__request_background_screenshot, mcp__canvas__get_zone_boundaries, mcp__interior-layout__save_semantic_plan, mcp__interior-layout__load_semantic_plan, mcp__canvas__register_variant
model: inherit
---
...
**变体目录创建**：Step 1 入场先调 `mcp__canvas__register_variant({mode: "blank", slugs: [variantSlug], designZoneId, summary: variantContext.variantDirection})`，Server 会创建 `variants/{variantSlug}/` 全部叶子目录骨架 + 初始空 modules.json（含 schemeMetadata.summary）。**禁止跳过此步骤**——后续 Step 2 save_semantic_plan / Step 3 写 modules.json 都依赖此目录骨架。

**先读后写**：修改前先 Read 当前内容，不凭猜测写入。**modules.json 写入用 `Write` / `Edit` 工具直接编辑**，文件形态 `{schemeMetadata: {summary}, modules: [...]}`：编辑 `modules` 数组时**必须保留外层 `schemeMetadata` 字段**（误删会让 Web 端变体 tooltip 丢失设计意图）。
...
- 入场必须先 `register_variant({mode: "blank", ...})` 创建变体目录骨架
...
1. **【必须】**首先调 `mcp__canvas__register_variant({designZoneId, slugs: [variantSlug], mode: "blank", summary: variantContext.variantDirection})`
   - Server 会创建 `variants/{variantSlug}/` 及所有叶子子目录 + 写入初始 `modules.json` 模板（含 `schemeMetadata.summary = variantContext.variantDirection`，`modules: []`）
   - 返回的 `leafPaths` 是各叶子 modules.json 的绝对路径，Step 3 写模块时用
   - **若返回 `errors` 含 `already-exists`** 表示主控可能重复派发，请用固定回复 `调度违规：variant-design-agent ...` 退出
2. `mcp__canvas__load_semantic_plan({zoneId: designZoneId, tag: "spatial-skeleton"})` —— **不传 variantId**
   - 取 canonical 的空间骨架（`spatial-skeleton` 是 canonical-only tag，所有 variant 共享）
3. 通读派发包中的 `variantContext` 4 字段（`variantDirection` / `variantNarrative` / `variantAnchorSeed` / `variantAvoidance`），理解本变体的设计方向
4. `mcp__canvas__get_zone_boundaries({zoneIds: [designZoneId]})` —— 取设计区与其叶子分区边界
...
1. 选定一个 `leafZoneId`，从 Step 1 `register_variant` 返回的 `leafPaths[leafZoneId]` 拿到该叶子 modules.json 绝对路径
2. `Read` 该 modules.json 拿到当前 wrapper（含 schemeMetadata）
3. 用 `Write` 整体重写 modules.json：
   ```json
   {
     "schemeMetadata": { "summary": "<保持 Step 1 写入的 variantContext.variantDirection 不变>" },
     "modules": [...完整模块数组...]
   }
   ```
   **必须保留 `schemeMetadata` 字段**——误删会让 Web 端变体 tooltip 丢失设计意图。
4. 验：
   ```text
   validate_layout({
     zoneIds: [leafZoneId],
   ...
   ```
5. `errorCount = 0` → ✅，进入下一个叶子分区
6. `errorCount > 0` → 修补循环（参见下文）
7. 全部叶子分区处理完毕 → Step 4
...
- 再次 `Write` 覆盖（保留 schemeMetadata）→ 再 `validate_layout`

**何时认输**：诊断在原地打转、或本变体的 `variantContext` 与几何空间不可兼得 → 在汇报中显式标注"本变体无法兑现 `variantContext`，已上报为自动改图建议"，**不要**强行写出违反 `variantContext` 的方案。

**【必须】**每个 `Write` modules.json 后立即 `validate_layout`；不验证就交付 = 调度违规。

**【必须】**`Write` modules.json 时必须包含 `schemeMetadata` 字段，summary 保持 Step 1 写入的 `variantContext.variantDirection`，不要清空也不要改写为其他内容。
...
3. `Write modules.json` 调用次数（按叶子分区列出）+ 各 validate 结果
4. 修补循环次数（如有）
...
- **【禁止】**用 `register_variant({mode: "clone-from-canonical" | "clone-from-variant"})` 触发 clone（你的 mode 固定为 `"blank"`；clone 是 module-relocation-agent 的语义）
- **【禁止】**写 sidecar `.meta.json` 文件
- **【禁止】**`Write` modules.json 时省略 `schemeMetadata` 字段（误删会让 Web 端变体设计意图丢失）
