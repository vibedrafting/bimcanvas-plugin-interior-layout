...
**【必须】**核对采纳方案是否已含房间策略允许的可选家具（如梳妆台 / 斗柜等，见**已注入的房间策略**「可选家具」节（Skill-L2 按 roomType 注入））。**若缺，尝试补入**：
1. `get_zone_boundaries` + 当前 modules → 算剩余可用墙段 / 空间。
2. 逐个可选家具做**闭合预检**：能否在**不侵占主家具 / 已声明留白 / 主通道 / 门禁（ez_*）**前提下塞入（采光型 vs 组合型按**已注入的房间策略**权衡 + 动线侵占核验）。
