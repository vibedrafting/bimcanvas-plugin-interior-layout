...
**【必须】**核对采纳方案是否已含房间策略允许的可选家具（如梳妆台 / 斗柜等）。**若缺，尝试补入**：
1. `get_zone_boundaries` + 当前 modules → 算剩余可用墙段 / 空间。
2. 逐个可选家具做**闭合预检**：能否在**不侵占主家具 / 已声明留白 / 主通道 / 门禁（ez_*）**前提下塞入（在采光好与组合便利间权衡、别挡通道）。**通道扣减口径见房间策略「床前通道检查」**：只扣家具**实体深度**，使用区与通行通道分时复用、不额外扣减（实测误判：400mm 深梳妆台被记成"400+使用区600=1000"双重扣减，把 1200−400=800 ≥800 的可行采光位算成不可行）。
3. **【必须】联动置换检查（空墙段全部不可行时不许跳过）**：执行房间策略「联动检查」——对照本方案收纳总延米与房型常规需求评估富余度，评估**缩短 / 置换一段主收纳**给可选家具腾位（如以梳妆台替换衣柜短腿一段、与衣柜组成工作站）。可行 → 记 `[自动改图建议]`（置换主家具属语义级，不静默执行）；不可行 → 在省略理由中写明联动检查结论（收纳延米多少、为何不可让）。
   > WHY：可选家具的最优位常被主收纳先占；只扫空墙段等于默认"主家具先到先得"，知识层的联动检查规则会因没有执行钩子而永远沉睡（实测连续两轮零触发）。
4. 能放 → 放最佳候选位、bounds 4 顶点、`Edit` 写入采纳叶子 `modules.json`、`validate_layout`；并 `Edit` 同步施工简报「可选/附属家具」节。
5. 不能（含联动检查也不可行）→ 在 `optimizationRecord` 记**坐标级省略理由**（哪面墙剩多少、被谁占、联动检查为何不可让），**禁 hand-wave**。
6. **边界**：补可选**不动主家具墙归属 / 不改方向 / 不侵占几何成立的留白（通道 / 操作净空 / 门禁）**——这与「不可静默执行」里的"新增主家具"无关，补可选是本分身正当职责。
7. **叙事性留白不是终点**：若可选家具仅被**叙事性留白**（"开阔感""释放某墙"之类非几何必需的声明）挡住、几何上其实放得下，本轮不强行落位，但**必须在 `optimizationRecord` 记 `[自动改图建议]`**（写明哪面墙几何可行、被哪条叙事声明挡住），**禁止静默放弃**——是否突破叙事性留白由用户/上游定夺，你的职责是把这个可行选项暴露出来。
