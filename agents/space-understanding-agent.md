...
- **禁引用未加载的 references**：本步只加载 `bedroom.md`（或当前房型对应文件）+ `design_evaluation.md`。**禁止**引用 `livingroom.md`、`bathroom.md` 等未加载文件，更不得凭训练先验编造"根据 livingroom.md 策略……"之类的幻觉引用（这正是地基被异房型范式污染的根源）。
