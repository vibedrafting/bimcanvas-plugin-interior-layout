...
- **【必须·`动线设计` 维：门净空区坐标交叠核验】**评 `动线设计` 维时（仅本维），**强制**取各门净空禁区 `ez_*b`（门前净空区，`get_zone_boundaries` 返回的 `exclusions` 几何，或 Read 项目 `computed/exclusions.json`；单位 mm，`rawBoundary` 为 4 顶点矩形）与各**可选家具**（梳妆台/斗柜等）的 `bounds` + 其**前向使用区**（朝向前方深度 ≥600mm 的操作/落座净空矩形）做**矩形交叠计算**。任一可选家具的 bounds 或前向使用区与某 `ez_*b` 交叠 → **必须出明确负向 finding**（带交叠坐标与 ez id，`score` 扣减），对应 bedroom.md 第 199-204 行"组合型紧邻衣柜但侵占主卫动线"反例。**不得**仅凭"视觉上还算通顺 / 紧邻衣柜算组合"等语义印象判该维通过——这是坐标可算的硬反例，按上一条"定量硬反例必明确认定"处理，不受截图成败影响。
- **【必须·`功能叙事` 维：窗侧空段坐标核验】**评 `功能叙事` 维时（仅本维），若床头墙含窗，**强制**坐标算窗帘占位结束线与最近睡眠组构件（窗侧床头柜/床）贴窗边的间距：>200mm 的无功能空段判违规（应 gap=0，对应 bedroom.md 第 115/298-300 行"睡眠组居中→窗帘盒与床之间无功能空段"），`score` 须体现扣减，并出带坐标的负向 finding。**不得接受**施工简报里"窗前通行留白 / 窗前缓冲"之类把该空段合理化的措辞——窗帘与床之间的空段不是有意留白。此项为纯坐标判定，不受截图成败影响。
