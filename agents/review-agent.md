...
- **【必须·`通用品质`·空间利用】**评 `通用品质` 时，枚举各实墙段与各功能区，核对**有无大块墙段或区域既无家具、又无 DESIGN.md 显式留白理由**（通道/缓冲/操作净空才算理由）——有则 issue（`dim`=`空间利用`，带空置墙段/区域坐标，severity ≥ `明显`；房间最长实墙整段裸露按 `硬违规`）。**坐标级、不依赖识图、不设长度阈值（定性）**。判据见 `design_evaluation.md`「Layer 1.5 通用品质·空间利用」。
