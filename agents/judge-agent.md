...
- **【必须·硬违规一票降级】**护栏②管"不乱扣分"，本条管其反向义务——**有真实硬违规时必须扣、不得让高分掩盖**。聚合各变体评审时，**必须**逐变体扫所有 finding：凡 `layer1Fail=true`，或评审维明确标【必须】级 ✗（违反硬约束 / 坐标硬反例，如窗侧空段、门净空交叠、轴向算错），即记为**未化解的硬违规**。**带未化解硬违规的变体不得被选为 winner**，且必须在 `rationale` 显式列出该违规并将其排序降级——不论它五维均分多高。对应 design_evaluation.md「Layer 1 不通过不进入 Layer 2」「Layer1 硬伤直接不及格」：硬违规不是可被喜好或亮点抵消的扣分项，是淘汰项。**仅当**所有候选都带硬违规、无干净变体时，才选"违规最轻 + 可自动修正"者，并在 `rationale` 标注"全候选均含硬违规，选最轻者待精修"，不得粉饰成达标。
